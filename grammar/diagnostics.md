...
| Code | Description | `extra` keys |
|------|-------------|--------------|
| **E0801** | callee's effects not covered by caller's declared set (B.1 + B.2). A function passed as a **value** counts as a callee: its effects run under the call it is handed to, so `apply(logIt, s)` from a `pure` caller is refused (`extra.via = "function_value"`). A pure function value adds nothing — `map(double, xs)` stays clean | `caller`, `callee`, `caller_effects`, `missing_effect`, optional `via` |
